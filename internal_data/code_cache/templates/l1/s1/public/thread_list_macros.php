<?php
// FROM HASH: 005f2fbb0bfc259ba15362903057da5a
return array('macros' => array('item' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="structItem structItem--thread' . ($__vars['thread']['prefix_id'] ? (' is-prefix' . $__templater->escape($__vars['thread']['prefix_id'])) : '') . ($__templater->method($__vars['thread'], 'isIgnored', array()) ? ' is-ignored' : '') . (($__templater->method($__vars['thread'], 'isUnread', array()) AND (!$__vars['forceRead'])) ? ' is-unread' : '') . (($__vars['thread']['sticky'] == '1') ? ' is-sticky' : '') . (($__vars['thread']['discussion_open'] == '0') ? ' is-locked' : '') . (($__vars['thread']['discussion_state'] == 'moderated') ? ' is-moderated' : '') . (($__vars['thread']['discussion_state'] == 'deleted') ? ' is-deleted' : '') . ' js-inlineModContainer js-threadListItem-' . $__templater->escape($__vars['thread']['thread_id']) . ' threadListItemIcon-' . $__templater->func('property', array('xbDiscussionIcon', ), true) . '" data-author="' . ($__templater->escape($__vars['thread']['User']['username']) ?: $__templater->escape($__vars['thread']['username'])) . '">
	
		';
	if ($__templater->func('property', array('xbDiscussionIcon', ), false) == 'replies') {
		$__finalCompiled .= '
		...
		if ($__vars['thread']['sticky']) {
			...
			';
		} else if ($__vars['thread']['discussion_type'] == 'redirect') {
			$__finalCompiled .= '
				' . $__templater->fontAwesome('fa-external-link-alt', array(
				'data-xf-init' => 'tooltip',
				'title' => 'Redirect',
			)) . '
			';
			...
			$__finalCompiled .= '
				' . $__templater->fontAwesome('fa-asterisk', array(
				'data-xf-init' => 'tooltip',
		...
		$__finalCompiled .= '
			<span data-xf-init="tooltip" title="' . 'Replies' . '">' . (($__vars['thread']['discussion_type'] == 'redirect') ? '&ndash;' : $__templater->filter($__vars['thread']['reply_count'], array(array('number', array()),), true)) . '</span>
		</div>
		';
	}
	$__finalCompiled .= '
		...
		';
	if ($__templater->func('property', array('xbDiscussionIcon', ), false) == 'icon') {
		$__finalCompiled .= '
	...
	$__finalCompiled .= '
		...
		';
	if ($__templater->func('property', array('xbDiscussionIcon', ), false) == 'default') {
		$__finalCompiled .= '
		...
		if ($__vars['thread']['discussion_type'] != 'redirect') {
			$__compilerTemp2 .= '
								<li class="structItem-extraInfoMinor">
									<a href="' . $__templater->func('link', array('threads/edit', $__vars['thread'], ), true) . '" data-xf-click="overlay" data-href="' . $__templater->func('link', array('threads/edit', $__vars['thread'], $__vars['editParams'], ), true) . '">
										' . 'Edit' . '
									</a>
								</li>
							';
	...
	if ($__vars['thread']['discussion_state'] == 'deleted') {
		...
		if ($__templater->func('property', array('xbDiscussionIcon', ), false) == 'replies') {
			$__finalCompiled .= '
							<li class="structItem-viewCount">' . 'Views' . ' - ' . (($__vars['thread']['discussion_type'] == 'redirect') ? '&ndash;' : (($__vars['thread']['view_count'] > $__vars['thread']['reply_count']) ? $__templater->filter($__vars['thread']['view_count'], array(array('number', array()),), true) : $__templater->func('number', array($__vars['thread']['reply_count'] + 1, ), true))) . '</li>
						';
		}
		$__finalCompiled .= '
					</ul>
